...
export default function PengawasPage() {
  ...
  // State Ganti Password / PIN Pengawas
  ...
  const handleSendComment = (laporanId: string, customText?: string) => {
    const text = customText || commentInput[laporanId];
    if (text && text.trim() && user) {
      addComment(laporanId, user, text.trim());
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPasswordMsg({ type: 'success', text: '✅ Password / PIN Pengawas berhasil diperbarui!' });
      ...
      <main className="p-3.5 space-y-3.5">
            ...
            <div className="space-y-4">
              {feedLaporans.map((lap) => (
                ...
                >
                  ...
                  {/* Pengawas Quick Presets & Comment Box */}
                  <div className="pt-3 border-t border-slate-100 space-y-2.5">
                    {/* Quick Presets for Pengawas */}
                    <div className="space-y-1">
                      <div className="text-[10px] font-bold text-slate-500 flex items-center gap-1">
                        <Sparkles className="w-3 h-3 text-amber-500" />
                        <span>Beri Apresiasi Pengawas (1 Sentuh):</span>
                      </div>
                      <div className="flex gap-1.5 overflow-x-auto pb-1 text-[10px] font-semibold">
                        {[
                          '✨ Mumtaz! Istiqomah selalu ananda.',
                          '🕌 Baarakallahu fiik, pertahankan hafalannya!',
                          '💪 Masya Allah, tingkatkan terus kedisiplinannya!',
                          '📖 Alhamdulillah, catatan laporan sangat baik.',
                        ].map((preset, idx) => (
                          <button
                            key={idx}
                            type="button"
                            onClick={() => handleSendComment(lap.id, preset)}
                            className="px-2.5 py-1 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300/80 shrink-0 transition active:scale-95 text-[10px]"
                          >
                            {preset}
                          </button>
                        ))}
                    ...
                    {lap.comments && lap.comments.length > 0 && (
                      <div className="space-y-1.5 bg-slate-50 p-2.5 rounded-2xl border border-slate-200/80">
                        <div className="text-[10px] font-bold text-slate-500 mb-1 flex items-center gap-1">
                          ...
                          <span>Riwayat Diskusi & Komentar ({lap.comments.length}):</span>
                        </div>
                        {lap.comments.map((c) => {
                          const isPengawas = c.role === 'PENGAWAS' || c.role === 'SUPER_ADMIN' || c.isi.includes('[Catatan Pengawas]');
                          const isMusyrif = c.role === 'MUSYRIF';
                            ...
                            >
                              <div className="flex items-center justify-between gap-1 mb-0.5">
                                <div className="flex items-center gap-1 flex-wrap">
                                  <span className="font-extrabold">{c.nama}</span>
                                  {isPengawas && (
                                    <span className="text-[8px] bg-amber-400 text-amber-950 px-1.5 py-0.2 rounded-full font-black">
                                      🛡️ Pengawas
                                    </span>
                                  )}
                                  {isMusyrif && (
                                    <span className="text-[8px] bg-teal-200 text-teal-950 px-1.5 py-0.2 rounded-full font-bold">
                                      ☪️ Musyrif
                                    </span>
                                  )}
                                </div>
                                <span className="text-[9px] text-slate-400">{c.waktu}</span>
                              </div>
                              <p className="text-[11px]">{c.isi.replace('[Catatan Pengawas] ', '')}</p>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="flex gap-1.5">
                      <input
                        type="text"
                        placeholder="Ketik bimbingan / catatan pengawas..."
                        ...
                        className="flex-1 text-xs px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 font-medium"
                      />
                      <button
                        type="button"
                        onClick={() => handleSendComment(lap.id)}
                        className="px-3.5 py-2 bg-sky-700 hover:bg-sky-800 text-white rounded-xl font-bold text-xs flex items-center justify-center transition shadow-xs active:scale-95"
                      >
        ...
        {activeTab === 'profil' && (
          <div className="space-y-3.5">
            ...
            {/* Informasi Identitas Pengawas Terkunci (Read-Only) */}
            <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm space-y-2.5">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <div className="flex items-center gap-1.5 font-bold text-xs text-slate-800">
                  ...
                  <span>Informasi Akun Pengawas</span>
                </div>
            ...
            {/* Formulir Ganti Password / PIN Pengawas */}
            <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm space-y-3">
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-800 pb-2 border-b border-slate-100">
                ...
                <span>Ganti Password / PIN Login Pengawas</span>
              </div>
              ...
              <form onSubmit={handleUpdatePassword} className="space-y-2.5 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 font-medium"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-sky-700 hover:bg-sky-800 disabled:bg-slate-300 text-white font-bold rounded-xl shadow-xs transition active:scale-95 flex items-center justify-center gap-1.5 mt-1"
                >
                  <Save className="w-3.5 h-3.5" />
                  <span>{isUpdatingPassword ? 'Menyimpan...' : 'Simpan Password Baru'}</span>
                </button>
              </form>
            </div>
